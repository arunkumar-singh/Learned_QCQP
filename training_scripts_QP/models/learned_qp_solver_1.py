...
class learned_qp_solver(nn.Module):
	...
	def compute_projection(self, lamda, vel_projected, vel_init, acc_init,  vel_samples, vel_max, vel_min, acc_max, acc_min, jerk_max, jerk_min, theta_min, theta_max, theta_init, s):
		...
		for i in range(0, self.maxiter):
			...
			fixed_point_residual = torch.linalg.norm(s_prev-s, dim = 1)+torch.linalg.norm(lamda_prev-lamda, dim = 1)+torch.linalg.norm(vel_projected_prev-vel_projected, dim = 1)

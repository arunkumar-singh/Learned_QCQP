...
class Learned_QCQP(nn.Module):
	...
	def __init__(self, num_obs, t_fin, P, Pdot, Pddot, point_net, num_batch, min_pcd, max_pcd, inp_mean, inp_std, gru_context, gru_hidden_state_init, mlp_pred, mlp_init):
	...
	def compute_boundary_layer_optim(self, init_state_ego):
		...
		b_eq_y = torch.hstack([y_init_vec, vy_init_vec, ay_init_vec, vy_fin_vec  ])
	...
	def compute_mat_inv_optim(self):
		...
		cost_mat_inv_x = torch.linalg.inv(cost_mat_x+0.00001*torch.eye( cost_mat_x.size(dim = 1), device = device ) )
		cost_mat_inv_y = torch.linalg.inv(cost_mat_y+0.00001*torch.eye( cost_mat_y.size(dim = 1), device = device ) )
	...
	def compute_alph_d(self, primal_sol, lamda_x, lamda_y, x_obs_circle_traj, y_obs_circle_traj, y_ub, y_lb, a_obs, b_obs ):
	...
	def compute_x(self, cost_mat_inv_x, cost_mat_inv_y, b_eq_x, b_eq_y, x_obs_circle_traj, y_obs_circle_traj, a_obs, b_obs, lamda_x, lamda_y, alpha_obs, d_obs, alpha_a, d_a, alpha_v, d_v, y_ub, y_lb, s_lane, c_x_pred, c_y_pred):
		...
		lincost_x = -lamda_x  - \
					...
					self.rho_projection *torch.mm(self.A_projection.T, c_x_pred.T).T
		...
		lincost_y = -lamda_y - \
					...
					self.rho_projection *torch.mm(self.A_projection.T, c_y_pred.T).T
		   
		# print(cost_mat_inv_x.dtype, lincost_x.dtype, b_eq_x.dtype)
	...
	def custom_forward(self, init_state_ego, primal_sol, x_obs_circle_traj, y_obs_circle_traj, a_obs, b_obs, lamda_x, lamda_y, y_ub, y_lb, h_0, c_x_pred, c_y_pred, accumulated_res_primal_list, accumulated_res_fixed_point_list) :	
		
		
		# # Boundary conditions
		b_eq_x, b_eq_y = self.compute_boundary_layer_optim(init_state_ego)
		...
		for i in range(0, self.maxiter):
			...
			primal_sol_prev = primal_sol.clone()
			lamda_x_prev = lamda_x.clone()
			lamda_y_prev = lamda_y.clone()
   
			lamda_prev = torch.hstack(( lamda_x_prev, lamda_y_prev  ))

			alpha_a, d_a, lamda_x, lamda_y, alpha_v, d_v, alpha_obs, d_obs, s_lane, res_norm_batch = self.compute_alph_d(primal_sol, lamda_x, lamda_y, x_obs_circle_traj, y_obs_circle_traj, y_ub, y_lb, a_obs, b_obs)
			lamda = torch.hstack(( lamda_x, lamda_y  ))	
 
			primal_sol = self.compute_x(cost_mat_inv_x, cost_mat_inv_y, b_eq_x, b_eq_y, x_obs_circle_traj, y_obs_circle_traj, a_obs, b_obs, lamda_x, lamda_y, alpha_obs, d_obs, alpha_a, d_a, alpha_v, d_v, y_ub, y_lb, s_lane, c_x_pred, c_y_pred)

			r_1 = torch.hstack(( primal_sol_prev, lamda_prev)) 
			r_2 = torch.hstack((primal_sol, lamda))
			...
			primal_sol_delta = gru_output[:, 0:2*self.nvar]
			lamda_delta = gru_output[:, 2*self.nvar : 4*self.nvar]
			primal_sol = primal_sol+primal_sol_delta
	...
	def compute_mat_inv_init(self):
		...
		cost_x = torch.mm(self.A_projection.T, self.A_projection)
		cost_y = torch.mm(self.A_projection.T, self.A_projection)      	 
	...
	def qp_layer_initialization(self, init_state_ego, primal_sol):
		...
		lincost_x = self.rho_projection * torch.mm(self.A_projection.T, primal_sol_x.T).T
			
		lincost_y = self.rho_projection * torch.mm(self.A_projection.T, primal_sol_y.T).T
	...
	def decoder_function(self, inp_norm, init_state_ego, pcd_scaled, x_obs_circle_traj, y_obs_circle_traj, a_obs, b_obs, y_ub, y_lb):
	 
		# PCD feature extractor
		pcd_features = self.point_net(pcd_scaled)
		inp_features_1 = torch.cat([pcd_features, inp_norm   ], dim = 1)


		c_pred = self.mlp_pred(inp_features_1)
		c_pred = self.qp_layer_initialization(init_state_ego, c_pred)

		c_x_pred = c_pred[:, 0: self.nvar]
		c_y_pred = c_pred[:, self.nvar:2*self.nvar]

		inp_features_2 = torch.cat([inp_features_1, c_pred   ], dim = 1)

		neural_output_batch = self.mlp_init(inp_features_2)
  
		h_0 = self.gru_hidden_state_init(inp_features_2)
  

		lamda_init = neural_output_batch[:, 0 : 2*self.nvar  ]

		primal_sol = neural_output_batch[:, 2*self.nvar : 4*self.nvar   ]
  
		# param_predict = neural_output_batch[:, 4*self.nvar: 4*self.nvar+2 ]
	
		primal_sol = self.qp_layer_initialization(init_state_ego, primal_sol)
		...
		primal_sol, accumulated_res_primal, accumulated_res_fixed_point, res_primal_stack, res_fixed_point_stack =  self.custom_forward(init_state_ego, primal_sol, x_obs_circle_traj, y_obs_circle_traj, a_obs, b_obs, lamda_x, lamda_y, y_ub, y_lb, h_0, c_x_pred, c_y_pred, accumulated_res_primal_list, accumulated_res_fixed_point_list)	
		
		return primal_sol, accumulated_res_primal, accumulated_res_fixed_point, res_primal_stack, res_fixed_point_stack, c_pred
	


	def ss_loss(self, accumulated_res_primal, accumulated_res_fixed_point, primal_sol, param_des, Pddot_diag, c_pred):
		...
		v_des = v_des[:, None]*torch.ones((self.num_batch, self.num), device=device)
		y_des = y_des[:, None]*torch.ones((self.num_batch, self.num), device=device)
		
		res_v = 0.5*self.rcl_loss(xdot, v_des )
		res_y = 0.5*self.rcl_loss(y, y_des )

		acc_loss = 0.5 * (torch.mean(predict_acc))

		
		obj_value = res_y+res_v+0.01*acc_loss

		proj_loss = 0.5*self.rcl_loss(primal_sol, c_pred)


		##########################################33333


		yaw = torch.arctan(ydot/xdot)

		yaw_rate = torch.hstack(( torch.zeros((self.num_batch, 1), device = device),    torch.diff(yaw, dim = 1) ))
		v = torch.sqrt(xdot**2+ydot**2+0.0001)
		steer = torch.arctan((yaw_rate/v)*self.wheel_base)
  
		# # curv_loss = 0.5*torch.mean(torch.linalg.norm(curvature, dim = 1))
  
		# curv_penalty = torch.maximum( torch.zeros(( self.num_batch, self.num  ), device = device), torch.abs(curvature)-torch.tan(self.steer_max*torch.ones((self.num_batch, self.num), device = device   )     )/self.wheel_base       ) 
		...
		# steer_penalty_ub = steer-self.steer_max*torch.ones((self.num_batch, self.num), device = device        ) 
		# steer_penalty_lb = -steer+self.steer_max*torch.ones((self.num_batch, self.num), device = device        ) 
		
		steer_penalty = steer_penalty_lb+steer_penalty_ub

		steer_loss = 0.5*torch.mean(torch.linalg.norm(steer_penalty, dim = 1))

		yaw_rate_loss = 0.5*torch.mean( torch.linalg.norm(yaw_rate, dim = 1))
	
		loss = fixed_point_loss+0.1*obj_value+yaw_rate_loss
 
		return loss, fixed_point_loss, primal_loss, obj_value, yaw_rate_loss
	
	def forward(self, inp, init_state_ego, pcd_data,  closest_obs, psi_obs, dim_x_obs, dim_y_obs,  y_ub, y_lb):
		...
		# Decode y
		primal_sol, accumulated_res_primal, accumulated_res_fixed_point, res_primal_stack, res_fixed_point_stack, c_pred = self.decoder_function(inp_norm, init_state_ego, pcd_scaled, x_obs_circle_traj, y_obs_circle_traj, a_obs, b_obs, y_ub, y_lb)
	 
		# predict_traj = (P_diag @ primal_sol.T).T
		# predict_acc = (Pddot_diag @ primal_sol.T).T

		# print(accumulated_res_primal[0])

		return primal_sol, accumulated_res_primal, accumulated_res_fixed_point, c_pred

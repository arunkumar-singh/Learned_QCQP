...
class Learned_QCQP(nn.Module):
	...
	def __init__(self, num_obs, t_fin, P, Pdot, Pddot, point_net, num_batch, min_pcd, max_pcd, inp_mean, inp_std, gru_context, gru_hidden_state_init, mlp_init):
		...
		self.A_eq_y = torch.vstack([self.P[0], self.Pdot[0], self.Pddot[0], self.P[-1]     ]  )
	...
	def compute_boundary_layer_optim(self, init_state_ego, goal_des):
		...
		b_eq_y = torch.hstack([y_init_vec, vy_init_vec, ay_init_vec, y_fin_vec  ])
	...
	def compute_x(self, b_eq_x, b_eq_y, x_obs_circle_traj, y_obs_circle_traj, a_obs, b_obs, lamda_x, lamda_y, alpha_obs, d_obs, alpha_a, d_a, alpha_v, d_v, y_ub, y_lb, s_lane):
		...
		cost_y = cost_x+self.rho_lane*torch.mm(self.A_lane.T, self.A_lane)
	...
	def custom_forward(self, init_state_ego, x_obs_circle_traj, y_obs_circle_traj, a_obs, b_obs, y_ub, y_lb, goal_des, c_x_guess, c_y_guess, h_0, lamda_x, lamda_y):	
		...
		for i in range(0, self.maxiter):
			...
			primal_delta = gru_output[:, 0: 2*self.nvar]
			
			# lamda_delta = gru_output[:, 2*self.nvar: 4*self.nvar]
			primal_sol = primal_sol+primal_delta
	...
	def decoder_function(self, inp_norm, init_state_ego, pcd_scaled, x_obs_circle_traj, y_obs_circle_traj, a_obs, b_obs, y_ub, y_lb, param_des, goal_des):
		...
		primal_sol, accumulated_res_primal, accumulated_res_fixed_point, res_primal_stack, res_fixed_point_stack =  self.custom_forward(init_state_ego, x_obs_circle_traj, y_obs_circle_traj, a_obs, b_obs, y_ub, y_lb, goal_des, c_x_guess, c_y_guess, h_0, lamda_x_init, lamda_y_init)	
